...
namespace LD_LotAdjustmentAssistant
{
    public partial class ucSummary : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucSummary(HMConnection.HMCon connection, TUC_HMDevXManager.TUC_HMDevXManager tuc)
        {
            ...
            ucSubledgerPreview = new Subledger_Preview.ucSubledger_Preview(hmCon, tuc, Subledger_Preview.ucSubledger_Preview.Flavor.LD_LotAdjustmentAssistant);
        ...
        private void btnReporting_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (HM_Report_Printer.frmHM_Report_Printer ReportPrinter =
                    new HM_Report_Printer.frmHM_Report_Printer(hmCon, tuc_mgr, saParam, "LD_SaleAdjRegX.rpt", HM_Report_Printer.frmHM_Report_Printer.DBFlavor.TR))
                {
        ...
        public bool Finish()
        {
            if (popup.ShowPopup("Are you sure you want to process the following lot adjustments and post to " + ucSubledgerPreview.GetPeriodName().Replace(ucSubledgerPreview.Period.ToString() + ", ", "") + " " + ucSubledgerPreview.Year.ToString() + " (Period " + ucSubledgerPreview.Period.ToString() + ")?", WS_Popups.frmPopup.PopupType.Yes_No) == WS_Popups.frmPopup.PopupResult.Yes)
            {
